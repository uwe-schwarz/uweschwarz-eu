...
- After the main JS/TS upgrade step, run `bun <skill-dir>/scripts/unpin-semver-ranges.mjs <repo-root>` to convert exact `x.y.z` specs and any stray `latest` tags into ranged versions like `^1.2.3`. If it updates any manifest, re-run the package manager install or update step so the lockfiles match.
- After the final JS/TS install or update pass, run `bun <skill-dir>/scripts/check-no-latest-specifiers.mjs <repo-root>`. Do not proceed until it reports that no tracked `package.json` or lockfile still contains `latest`.
...
- Stop and report unsupported ecosystems/tooling, unsafe unrelated changes, failed required gates, or access/policy blockers. Preserve completed work without bypassing the blocker.
...
Usage:
```bash
bun /absolute/path/to/upgrade-dependencies-pr/scripts/unpin-semver-ranges.mjs /path/to/repo
```
...
Usage:
```bash
bun /absolute/path/to/upgrade-dependencies-pr/scripts/check-no-latest-specifiers.mjs /path/to/repo
```
