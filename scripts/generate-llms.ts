  ...
  try {
    ...
    const llmsKeywords = content.llms?.keywords ?? {
      de: [
        ...
        ...content.hero.titleElements.map((t) => t.de),
        ...content.skills.slice(0, 10).map((s) => s.name.de),
      ].join(", "),
      en: [
        content.siteMetadata.author,
        ...content.hero.titleElements.map((t) => t.en),
        ...content.skills.slice(0, 10).map((s) => s.name.en),
      ].join(", "),
    };
