...
interface FlaskIconHandle {
  startAnimation: () => void;
  stopAnimation: () => void;
}

interface FlaskIconProps extends HTMLAttributes<HTMLDivElement> {
  ref?: Ref<FlaskIconHandle>;
  size?: number;
}

const FLASK_GROUP_VARIANTS: Variants = {
  animate: {
    rotate: [0, 5, -5, 3, -3, 0],
    transition: {
      ease: "linear",
...
const FLASK_PATH_VARIANTS: Variants = {
  animate: {
    rotate: [0, 6, -6, 3, -3, 0],
    scale: 0.9,
    transition: {
...
function FlaskIcon({ className, onMouseEnter, onMouseLeave, ref, size = 28, ...props }: FlaskIconProps) {
  ...
  return (
    <div className={cn(className)} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props}>
      <LazyMotion features={domAnimation}>
        ...
        >
          <m.g animate={controls} initial="normal" variants={FLASK_GROUP_VARIANTS}>
