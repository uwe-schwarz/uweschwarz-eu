  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 dark:bg-gray-900/80 backdrop-blur-md shadow-sm' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <a href="#hero" className="text-2xl font-display font-bold text-foreground">
          <span className="text-gradient text-4xl">Oldman</span>
        </a>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center space-x-6">
          {siteContent.navigation.map(item => <a key={item.href} href={item.href} className="text-lg font-medium text-foreground hover:text-primary transition-colors link-underline">
              {t(item.label)}
            </a>)}
        </nav>
        ...
        <div className="hidden md:flex items-center space-x-2">
          {/* Language Toggle */}
          <Button variant="ghost" size="sm" onClick={toggleLanguage} className="text-foreground hover:text-primary">
            {language === 'en' ? 'DE' : 'EN'}
          </Button>
          
          {/* Theme Toggle */}
          <Button variant="ghost" size="icon" onClick={toggleTheme} className="text-foreground hover:text-primary" aria-label={t(theme === 'light' ? siteContent.translations.themeSwitch.light : siteContent.translations.themeSwitch.dark)}>
            ...
            <SheetContent side="left" className="w-full bg-background dark:bg-gray-900 p-0">
              <div className="flex flex-col h-full">
                ...
                <div className="flex-1 flex flex-col justify-center items-center space-y-6 py-8">
                  {siteContent.navigation.map(item => <a key={item.href} href={item.href} className="text-xl font-medium text-foreground hover:text-primary transition-colors">
                      {t(item.label)}
                    </a>)}
                </div>
                ...
                <div className="border-t border-gray-200 dark:border-gray-800 p-6 flex gap-4">
                  ...
                  <Button variant="outline" className="flex-1" onClick={toggleTheme}>
                    {theme === 'light' ? <><Moon size={16} className="mr-2" /> Dark</> : <><Sun size={16} className="mr-2" /> Light</>}
                  </Button>
                </div>
              </div>
            </SheetContent>
